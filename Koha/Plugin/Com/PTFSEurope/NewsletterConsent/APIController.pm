...
sub get_sync_upstream {
    ...
    return try {
        ...
        return $c->render(
            ...
            if ( $patron_sync->{error} eq 'no borrowernumber found'
            || $patron_sync->{error} eq 'no notice_email_address found' );
...
sub process_sync_mailchimp {
    ...
    my $request = HTTP::Request->new(
        'POST',
        $baseurl . '/lists/' . $mailchimp_list_id . '?skip_merge_validation=true&skip_duplicate_check=false'
    );
    ...
    if ( $patron_consent_status eq 'unsubscribed' ) {
        ...
        my $request = HTTP::Request->new(
            'DELETE',
            $baseurl . '/lists/' . $mailchimp_list_id . '/members/' . $mailchimp_contact_id
        );
